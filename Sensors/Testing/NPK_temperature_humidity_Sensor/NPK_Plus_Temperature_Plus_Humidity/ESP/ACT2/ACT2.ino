...
byte GetNitrogen(){
  ...
  times++;
  if( (values < LowRng && times < reps) || (values > UppRng && times < reps)) {Serial.println("REP N");goto NStart;}
  return values[4];
}
...
byte GetPhosphorous(){
  ...
  times++;
  if( (values < LowRng && times < reps) || (values > UppRng && times < reps)) {Serial.println("REP K"); goto KStart;}
  return values[4];
}
...
byte GetPotassium(){
  ...
  times++;
  if( (values < LowRng && times < reps) || (values > UppRng && times < reps)) {Serial.println("REP P");goto PStart;}
  return values[4];
}
